...
`list.expend()` 扩展原列表,可以加入列表或者元祖.
> 写列表或者元祖最后一项要多加一个","号 

`list.index()` 获取索引值(下标)

`list.insert(位置,项)` 往指定位置插入项 

`list.pop(index)` 移除某一项,并返回移除值(默认最后一项)

`list.remove(value)` 按值删除匹配到的第一项,与pop()不同的是remove()没有返回值.

`list.reverse()` 翻转原列表,会改变原列表.

`list.sort(规则)` 排序,会改变原列表.
************
### 字典内部功能
```
...
`dict.fromkeys(列表,值)` 从列表中获取键,然后赋值,创建新字典.

`dict.get(键,arg)` 通过键获取值,与dict[键]不同的是,如果键不存在不会报错,而是返回none.也可以传参数代替none.
...
`dict.values()` 返回所有值.
```
`dict.pop("keys",arg)` 通过键(必须)删除键值对,返回匹配值.若键不存在,返回arg参数,若没设置arg就报keyError.

`dict.update({字典})` 更新原字典,类似列表的extend,拼接.
